...
# Minimaal TOTAAL aantal pixels per kleur (grove ruisfilter). Oranje licht
# dunner/fragmentarischer op dan blauw, dus die krijgt een lagere drempel.
MIN_PIXELS_BLUE = 400
MIN_PIXELS_ORANGE = 200
...
# Oranje krijgt ook hier een lagere drempel omdat de lijn dunner is.
MIN_CLUSTER_AREA_BLUE = 150
MIN_CLUSTER_AREA_ORANGE = 80

# --- Stuur parameters ---
# De stuursterkte schaalt mee met het hoekverschil tussen de twee lijnen.
# Klein hoekverschil -> zacht sturen (STEER_MIN), groot hoekverschil -> STEER_MAX.
# Zet STEER_MIN en STEER_MAX gelijk om het meeschalen uit te zetten.
STEER_MIN = 0.2           # minimale stuurwaarde zodra er een geldige bocht is
STEER_MAX = 1.0           # maximale stuurwaarde (bij groot hoekverschil)
...
# --- Bocht-afronding (vasthouden) ---
# Als de kleuren uit beeld verdwijnen zit de auto vaak nog midden in de bocht.
# Dan HOUDT hij de laatst geziene stuurrichting vast gedurende TURN_HOLD
# seconden. Belangrijk: dit blokkeert NIET - de control-thread blijft gewoon
# frames verwerken en kijken. Ziet hij binnen die tijd een nieuwe geldige
# bocht, dan neemt die meteen over (en reset de timer). Pas als TURN_HOLD
# voorbij is zonder nieuwe detectie, gaat hij terug naar rechtdoor.
TURN_HOLD = 2.0
...
# Oranje - twee bereiken (A OR B) wegens fisheye-kleurverschuiving
lower_orange_a = np.array([168, 83, 147])
upper_orange_a = np.array([179, 120, 188])

lower_orange_b = np.array([0, 31, 129])
upper_orange_b = np.array([11, 68, 153])
...
# ==========================
def line_angle(mask, min_pixels):
    """
    ...
    min_pixels : minimaal aantal pixels nodig om een lijn te fitten.
...
def control_thread():
    ...
    # Laatst geziene stuurwaarde + tijdstip onthouden voor het vasthouden.
    # hold_until = tijdstip (time.time) tot wanneer we de richting vasthouden.
    last_steer = 0.0
    hold_until = 0.0
    ...
    while not stop_event.is_set():
        ...
        # Oranje uit twee bereiken (A OR B)
        ...
        # ==========================
        mask_blue = filter_small_clusters(mask_blue, MIN_CLUSTER_AREA_BLUE)
        mask_orange = filter_small_clusters(mask_orange, MIN_CLUSTER_AREA_ORANGE)

        # ==========================
        # Lijn fitten + hoek bepalen per kleur
        # ==========================
        blue_angle, blue_count, blue_fit = line_angle(mask_blue, MIN_PIXELS_BLUE)
        orange_angle, orange_count, orange_fit = line_angle(mask_orange, MIN_PIXELS_ORANGE)
        ...
        if blue_angle is not None and orange_angle is not None:
            ...
            if abs(diff) >= ANGLE_DEADZONE:
                ...
                last_steer = steering
                # Nieuwe geldige bocht -> vasthoud-timer (her)starten.
                hold_until = time.time() + TURN_HOLD

        # ==========================
        # Servo-bereik: car.steering accepteert -1.0 .. 1.0. We clampen op die
        # natuurlijke grens (de oude 0.8-begrenzing is vervallen nu de servo
        # weer origineel is).
        # ==========================
        ...
        # ==========================
        # BOCHT VASTHOUDEN (niet-blokkerend)
        # Geen geldige bocht dit frame. Als we nog binnen het vasthoud-venster
        # zitten (hold_until in de toekomst) en er een richting onthouden is,
        # blijven we die richting sturen. De lus draait gewoon door, dus de
        # auto blijft kijken en kan de volgende bocht herkennen (die wint dan
        # bovenin direct, want valid_turn zet steering opnieuw).
        # Is het venster voorbij, dan terug naar rechtdoor.
        # ==========================
        if not valid_turn:

            if last_steer != 0.0 and time.time() < hold_until:
                # Nog binnen het venster -> richting vasthouden
                steering = max(-1.0, min(1.0, last_steer))
                throttle = THROTTLE_TURN
                action_label = "BOCHT VASTHOUDEN"
                car.steering = steering
                motor_kit.motor3.throttle = throttle
            else:
                # Venster voorbij (of niets te houden) -> rechtdoor
                steering = 0.0
                throttle = THROTTLE_DRIVE
                last_steer = 0.0
                car.steering = steering
